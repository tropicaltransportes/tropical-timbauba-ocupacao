...
import { Activity, Building2, CalendarDays, ChevronRight, CircleHelp, Info, RefreshCw, Settings2 } from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <img className="logo logo-light" src={`${import.meta.env.BASE_URL}assets/tropical-white.png`} alt="Tropical Transportes" />
      <div className="sidebar-label">MONITOR OPERACIONAL</div>
      <nav>{[['Ocupação Timbaúba', Activity], ['Rotas', Building2], ['Sobre os dados', CircleHelp]].map(([label, Icon]) => <button key={label} className={activeTab === label ? 'nav-item active' : 'nav-item'} onClick={() => setActiveTab(label)}><Icon size={19} strokeWidth={2.1} /><span>{label}</span>{activeTab === label && <ChevronRight size={16} className="nav-arrow" />}</button>)}</nav>
      <div className="sidebar-footer"><span>Fonte dos dados</span><strong>Google Sheets</strong><small>CSV publicado pela<br />Tropical Transportes</small></div>
    </aside>
    <main className="main-content">
      ...
      <section className="content">
        ...
        {activeTab === 'Ocupação Timbaúba' && <>
          <section className="overview-card"><div className="gauge-block"><div className="gauge" style={{ '--gauge': `${summary.average * 1.8}deg` }}><div className="gauge-value">{summary.average}%</div></div><div className="gauge-caption"><strong>Ocupação das rotas regulares</strong><span>{summary.totalUsed.toLocaleString('pt-BR')} passageiros <i>/</i> {summary.totalCapacity.toLocaleString('pt-BR')} lugares</span><small>Indicador principal da operação estruturada</small></div></div><div className="divider"></div><div className="situation"><h3>SITUAÇÃO DAS ROTAS REGULARES</h3>{[['comfortable', 'Confortável (até 60%)', summary.comfortable], ['attention', 'Atenção (61% a 80%)', summary.attention], ['high', 'Alta ocupação (81% a 95%)', summary.high], ['critical', 'Crítica (acima de 95%)', summary.critical]].map(([key, label, count]) => <div className="situation-row" key={key}><span className={`legend-dot ${key}`}></span><span>{label}</span><b>{count} {count === 1 ? 'rota' : 'rotas'}</b></div>)}<div className="variable-note"><strong>Rotas variáveis: J03, P06 e P08</strong><span>Essas rotas têm baixa demanda esperada e configuração sujeita a mudanças. Por isso, são demonstradas separadamente e não compõem o indicador principal.</span><b>{summary.variableUsed.toLocaleString('pt-BR')} passageiros <i>/</i> {summary.variableCapacity.toLocaleString('pt-BR')} lugares</b></div></div></section>
        ...
        {activeTab === 'Rotas' && <section className="route-directory"><div className="directory-heading"><div><p className="eyebrow">REFERÊNCIA OPERACIONAL</p><h2>Rotas da operação Timbaúba</h2><p>Consulte o veículo, a ocupação atual e todos os pontos de embarque previstos no roteiro vigente.</p></div><div className="directory-summary"><strong>{REGULAR_ROUTES.length} rotas regulares</strong><span>J03, P06 e P08 aparecem separadamente como configurações variáveis.</span></div></div><div className="route-cards">{[...REGULAR_ROUTES.map((item) => ({ ...item, category: 'regular' })), ...VARIABLE_ROUTES.map((item) => ({ ...item, category: 'variable' }))].map((route) => { const live = rows.find((row) => row.route === route.route); const expanded = expandedRoutes.has(route.route); const [, kind] = live ? statusFor(live.occupancy) : ['Sem leitura', 'comfortable']; return <article className={`route-card ${route.category === 'variable' ? 'variable' : ''} ${expanded ? 'expanded' : ''}`} key={route.route}><div className="route-card-top"><div><span className="route-code">{route.route}</span><span className="route-city">{route.city}</span><span className={`route-badge ${route.category === 'variable' ? 'variable' : ''}`}>{route.category === 'variable' ? 'VARIÁVEL' : 'REGULAR'}</span></div>{live ? <strong className={`route-occupancy ${kind}`}>{live.occupancy}%</strong> : <strong className="route-occupancy empty">—</strong>}</div><div className="route-meta"><span><b>Veículo</b> {route.category === 'variable' ? 'configuração variável' : route.vehicle}</span><span><b>{route.category === 'variable' ? 'Referência' : 'Capacidade'}</b> {route.capacity} lugares</span></div>{route.category === 'regular' ? <><div className="route-path"><span><b>Partida</b>Garagem Tropical</span><span><b>Destino</b>Timbaúba S.A.</span></div><button className="route-toggle" type="button" aria-expanded={expanded} onClick={() => setExpandedRoutes((current) => { const next = new Set(current); next.has(route.route) ? next.delete(route.route) : next.add(route.route); return next; })}>{expanded ? 'Ocultar pontos e horários' : `Ver ${route.stopCount} pontos e horários`}<ChevronRight size={16} className={expanded ? 'rotate-90' : ''} /></button>{expanded && <div className="stops-list">{route.stops.map((stop) => <div className="stop-row" key={`${route.route}-${stop.number}-${stop.time}`}><span className="stop-number">{stop.number}</span><span className="stop-name">{stop.name}</span><time>{stop.time}</time></div>)}</div>}</> : <div className="variable-description"><strong>Configuração variável</strong><span>Baixa demanda esperada e roteiro sujeito a alterações operacionais.</span></div>}<div className="route-card-footer"><span>{live ? `${live.used} passageiros transportados` : 'Sem leitura atual'}</span><small>Atualização: {live ? `${formatDate(live.timestamp)} às ${formatTime(live.timestamp)}` : 'aguardando dados'}</small></div></article>; })}</div></section>}
        {activeTab === 'Sobre os dados' && <section className="placeholder panel"><p className="eyebrow">AVISO AO CONTRATANTE</p><h2>Versão experimental</h2><p>Este painel está em versão experimental e deverá passar por alterações, ajustes e melhorias ao longo da evolução da operação.</p><p>Os dados apresentados são baseados na leitura mais recente disponível e podem sofrer atualizações conforme os roteiros e os registros operacionais forem revisados.</p><button className="back-button" onClick={() => setActiveTab('Ocupação Timbaúba')}>Voltar para ocupação</button></section>}
